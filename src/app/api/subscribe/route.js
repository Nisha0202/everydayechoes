...
export async function POST(req) {
  try {
    ...
    console.log("POST request data:", { email, otp, name, step }); // Add this line for debugging
    ...
    //goggle login
    if (step === 4) {
      // Create a new document with the `email`, `name`, and `verified` fields, and exclude the `otp` field
      const newData = {
        email,
        name,
        verified: true,
    ...
    return NextResponse.json({ error: 'Failed to process subscription', details: error.message }, { status: 500 });
  }
